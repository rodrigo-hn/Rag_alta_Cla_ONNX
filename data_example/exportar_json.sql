-- =====================================================
-- EXPORTAR JSON A ARCHIVO CON FORMATO (Pretty Print)
-- Genera laboratorios_resultado.json con indentacion
-- =====================================================
-- USO:
--   Opcion 1 (desde sqlplus): @exportar_json.sql
--   Opcion 2 (desde terminal):
--     sqlplus -s system/Oracle123@ORCLPDB1 @exportar_json.sql > output.json
--     python3 -m json.tool output.json > laboratorios_resultado.json
-- =====================================================
...
FROM (
    WITH datos_pruebas AS (
        SELECT
            ...
            -- Conversion segura usando DEFAULT ON CONVERSION ERROR (Oracle 12.2+)
            TO_NUMBER(
